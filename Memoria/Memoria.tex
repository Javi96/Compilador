...
Las definiciones regulares correspondientes a las clases léxicas definidas son:
	...
	\textbf{(\mystar ) VAR:} LETRA([LETRA|DIG|\textbackslash \_]*)
	...
	\textbf{(\mystar ) NUMR:} SIGNO?(DIG+(DEC)?(EXP)?) 
	
	\tab \textbf{SIGNO:} [\textbackslash +,\textbackslash -] 
	
	\tab \textbf{DIG:} [0-9] 
	
	\tab \textbf{DEC:} (\textbackslash .)DIG+ 
	
	\tab \textbf{EX:} [e|E](SIGNO?DIG+(DEC)?)	
